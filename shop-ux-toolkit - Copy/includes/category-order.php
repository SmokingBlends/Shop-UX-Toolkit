...
add_action('product_cat_add_form_fields', 'ccow_add_category_order_field');
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
function ccow_add_category_order_field() {
    wp_nonce_field('custom_category_order', 'custom_category_order_nonce');
    ?>
...
add_action('product_cat_edit_form_fields', 'ccow_edit_category_order_field', 10, 2);
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
function ccow_edit_category_order_field($term, $taxonomy) {
    wp_nonce_field('custom_category_order', 'custom_category_order_nonce');
...
// Save the field
add_action('created_product_cat', 'ccow_save_category_order');
add_action('edited_product_cat', 'ccow_save_category_order');
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
function ccow_save_category_order($term_id) {
    $nonce = isset($_POST['custom_category_order_nonce']) ? sanitize_text_field(wp_unslash($_POST['custom_category_order_nonce'])) : '';
    if (!$nonce || !wp_verify_nonce($nonce, 'custom_category_order')) {
        return;
    }
...
add_filter('woocommerce_product_subcategories_args', 'ccow_custom_category_order_args');
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
function ccow_custom_category_order_args($args) {
    $args['custom_category_order'] = true;
...
add_filter('get_terms', 'ccow_custom_sort_product_categories', 10, 3);
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
function ccow_custom_sort_product_categories($terms, $taxonomies, $args) {
    if (!in_array('product_cat', (array) $taxonomies) || empty($args['custom_category_order'])) {
        return $terms;
    }
