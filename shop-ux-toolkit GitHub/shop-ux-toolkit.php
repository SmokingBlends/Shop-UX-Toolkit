...
const GPT_SB_SUX_DEFAULTS = [
  ...
  'blog_excerpts_enabled' => false, // New option for blog excerpts feature
];
...
function gpt_sb_sux_opt($key) {
  $o = gpt_sb_sux_opts();
  return isset($o[$key]) ? (bool) $o[$key] : (GPT_SB_SUX_DEFAULTS[$key] ?? null);
}
...
add_action('wp_enqueue_scripts', function () {
  ...
  // 5) Reviews-specific styles (always load if reviews file is active; no toggle for now)
