...
add_action( 'template_redirect', 'gpt_sb_sux_handle_meta_checkout' );
function gpt_sb_sux_handle_meta_checkout() {
    if (!gpt_sb_sux_opt('enable_facebook_cart')) {
        return;
    }
    // Only process on specific page with 'products' param
    if ( ! is_page('secure-checkout') || ! isset( $_GET['products'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        return;
    }
